...
export default function Members() {
  ...
  const getMemberStats = (memberId: string, memberCreatedAt: string) => {
    ...
    // Set cutoff date at Dec 31, 2025
    const cutoffDate = new Date("2025-12-31");

    // Use the earlier of cutoffDate or today
    const lastMonth = joinDate > cutoffDate ? joinDate : cutoffDate;

    // Calculate months from joinDate till lastMonth
    const monthsSinceJoining =
      (lastMonth.getFullYear() - joinDate.getFullYear()) * 12 +
      (lastMonth.getMonth() - joinDate.getMonth()) +
      1; // +1 to include last month

    // Expected collections till cutoff
    ...
    // Pending collections till Dec 2025
    const monthlyPending = Math.max(0, expectedCollections - totalCollected);

    // Loan pending with interest (still can use today as end date)
    const now = new Date();
    const loanPending = memberLoans.reduce((sum, loan) => {
      ...
      const accruedInterest =
        (principal * interestRate * daysElapsed) / (100 * 30);
                      ...
                      <div className="grid grid-cols-2 gap-3 mt-2">
                        <div className="bg-warning/10 rounded-lg p-2 text-center">
                          <p className="text-lg font-semibold text-warning">
                            {formatCurrency(stats.monthlyPending)}
                          </p>
                          <p className="text-xs text-muted-foreground">
                            Monthly Pending
                          </p>
                        ...
                        <div className="bg-destructive/10 rounded-lg p-2 text-center">
                          <p className="text-lg font-semibold text-destructive">
                            {formatCurrency(stats.loanPending)}
                          </p>
                          <p className="text-xs text-muted-foreground">
                            Loan + Interest
                          </p>
